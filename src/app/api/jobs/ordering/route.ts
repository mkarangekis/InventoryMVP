...
async function runOrderingLogic(sql: Sql, tenantId: string, locationId: string | null, runDate: string) {
  await sql`
  ...
  for (const row of candidates) {
    ...
    if (projectedRemaining >= row.reorder_point_oz) continue;
  ...
  for (const [key, lines] of linesByVendor.entries()) {
    ...
    const [po] = await sql`
      ...
      values (${tenant}, ${location}, ${vendorId}, 'draft', now())
      returning id
    `;
    ...
    for (const line of lines) {
      await sql`
        insert into purchase_order_lines (
          ...
          ${line.tenant_id}, ${po.id}, ${line.inventory_item_id},
          ${line.qty_units}, ${line.unit_size_oz}, ${line.unit_price}, ${line.line_total}
        )
      `;
    }
  }

  return linesByVendor.size;
}
...
export async function POST(request: Request) {
  ...
  if (internalSecret) {
    ...
    try {
      const [jobRun] = await sql`
        ...
        values (${tenantId}, ${locationId}, 'generate-draft-pos', 'running') returning id
      `;
      const count = await runOrderingLogic(sql, tenantId, locationId, runDate);
      await sql`update job_runs set status='completed',finished_at=now() where id=${jobRun.id}`;
      return Response.json({ created: count, message: "Draft purchase orders generated." });
    } catch (err) {
  ...
  if (isDemoEmail(userData.user.email)) {
    return Response.json({ created: 2, message: "Demo: draft purchase orders generated." });
  }
  ...
  try {
    ...
    console.info("api.jobs.ordering done", { count });
