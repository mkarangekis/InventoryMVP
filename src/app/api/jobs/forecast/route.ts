...
async function runForecastLogic(sql: Sql, tenantId: string, locationId: string | null, runDate: string) {
  await sql`
  ...
  await sql`
    with history as (
      ...
      where tud.usage_date>=${runDate}::date - ${FORECAST_HISTORY_DAYS} * interval '1 day'
        and tud.usage_date<${runDate}::date and tud.tenant_id=${tenantId}
    ...
    overall_stats as (
      select inventory_item_id,location_id,tenant_id,avg(ounces_used) as avg_oz
      from history group by inventory_item_id,location_id,tenant_id
    ),
    forecast_dates as (
    ...
    expanded as (
      ...
      from overall_stats os cross join forecast_dates fd
      left join weekday_stats ws on ws.inventory_item_id=os.inventory_item_id and ws.location_id=os.location_id
        and ws.tenant_id=os.tenant_id and ws.dow=extract(dow from fd.forecast_date)
    )
    ...
    select tenant_id,location_id,forecast_date,inventory_item_id,
      case when coalesce(day_count,0)>=3 then weekday_avg else overall_avg end,'dow_baseline',now()
    from expanded
  `;
}
...
export async function POST(request: Request) {
  ...
  // Internal call from automated POS imports
  ...
  if (internalSecret) {
    ...
    try {
      const [jobRun] = await sql`
        ...
        values (${tenantId}, ${locationId}, 'forecast', 'running') returning id
      `;
      await runForecastLogic(sql, tenantId, locationId, runDate);
      await sql`update job_runs set status='completed',finished_at=now() where id=${jobRun.id}`;
      return Response.json({ message: "Forecast generated." });
    } catch (err) {
  ...
  if (isDemoEmail(userData.user.email)) {
    return Response.json({ ok: true, message: "Demo: forecast job queued." });
  }
