...
export default function LoginClient({
  ...
  const postAuthTarget = operationsLogin ? "/operations" : null;
  const isOperationsLogin = operationsLogin;
  ...
  const handleAuth = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (errorMessage) {
      ...
      if (postAuthTarget === "/operations") {
        setStatus("Signed in. Verifying Operations access...");
        const operationsResponse = await fetch("/api/v1/operations/access", {
          headers: { Authorization: `Bearer ${token}` },
          cache: "no-store",
        });
        if (operationsResponse.ok) {
          setStatus("Access verified. Opening Operations Center...");
          redirectAfterAuth("/operations");
          return;
        }

        setStatus(operationsAccessFailureMessage(operationsResponse.status));
        setLoading(false);
        return;
      }
      ...
      <div className="auth-container">
        ...
        <div className="auth-card">
          ...
          <p className="auth-subtitle">
            {isOperationsLogin
              ? "Sign in with your dedicated Operations administrator account."
              : mode === "signup"
          ...
          {isOperationsLogin ? (
            <div className="auth-footer">
              <span>Access is restricted to approved administrators.</span>
            </div>
          ) : (
            <div className="auth-footer">
              {mode === "signin" ? (
